<script lang="ts">
  ...
  function handleConfirm() {
    console.log(
      "✅ Graduation dialog confirmed with graduateStudents:",
      graduateStudents
    );
      ...
      <Card class="border-0 shadow-none">
        <CardHeader class="pb-4">
          ...
          <p class="text-sm text-gray-600 mt-2">
            Choose whether to graduate senior students when archiving this
            event.
          </p>
        </CardHeader>
        ...
        <CardContent class="space-y-6">
          ...
          {#if students.length > 0}
            <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
              <h3 class="font-medium text-blue-900 mb-3 flex items-center">
                ...
                Found {students.length} Grade 12 Student{students.length === 1
                  ? ""
                  : "s"}
              </h3>

              <div
                class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-32 overflow-y-auto"
              >
                {#each students as student}
                  <div
                    class="text-sm text-blue-800 bg-white rounded px-3 py-2 border border-blue-200"
                  >
                    {student.firstName}
                    {student.lastName}
                  </div>
                ...
                <div class="flex items-start">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-yellow-700 mt-1">
                      They will appear in future event creation and student
                      lists. You can graduate them manually later.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200">
            <button
              ...
              class="px-6 py-2 border border-gray-300 hover:bg-gray-50 text-gray-700 rounded-md font-medium"
            >
              Cancel
            </button>
            <button
